...
def read_standard_ascii_DC_table(file_path, page=False, time_unit='hrs', angle_unit='deg', alpha_min=0.0):
...
def read_master_table(file_path,page=False):
    ...
    for i,l in enumerate(lines):
        ...
        if '#' not in l:
            ...
            frac_fs_W = float(entries[66])
            frac_fs_Z = float(entries[67])
            Ftotal_W = true_fs_W / frac_fs_W
            Ftotal_Z = true_fs_Z / frac_fs_Z
            fb_W = Ftotal_W - true_fs_W
            fb_Z = Ftotal_Z - true_fs_Z
